...
function chainPromises(promisesArray, action) {
  const arr = [];
  const syncChain = promisesArray.reduce(
    (chain, promise) => chain
      .then(() => promise).then((number) => arr.push(number)).catch(() => null),
    Promise.resolve(),
  );

  return syncChain.then(() => arr.reduce(action));
}
